...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        #self.keys = {l : Button(l, lambda c: c)  for l in LOWERCASE_LETTERS}  # Try a dictionary comprehension!
    ...
    def type(self, word):
        ...
        "*** YOUR CODE HERE ***"
        #self.typed += [self.keys[x].press().letter for x in word]
        for x in word:
            self.keys[x].press()
...
class WinkingBear(Bear):
    ...
    def __init__(self):
        "*** YOUR CODE HERE ***"
        super().__init__()
        self.close_next_eye = False

    def next_eye(self):
        "*** YOUR CODE HERE ***"
        self.close_next_eye = not self.close_next_eye
        return Eye(self.close_next_eye)
