...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        word_pair = (typed, source)
        if word_pair in cache and cache[word_pair][1] >= limit:
            return cache[word_pair][0]
        else:
            result = diff_function(typed, source, limit)
            cache[word_pair] = [result, limit]
            return result
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diffs = [abs(diff_function(typed_word, word, limit)) for word in word_list]
        min_diff = min(diffs)
        if min_diff > limit:
            return typed_word
        else:
...
def furry_fixes(typed, source, limit):
    ...
    if len_typed == 0 or len_source ==0:
        ...
        first_diff = 0 if typed[0] == source[0] else 1
        return diff + first_diff + furry_fixes(typed[1:len_min], source[1:len_min], limit - diff - first_diff)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed == source: # Feel free to remove or add additional cases
        ...
        if limit <= 0:
            return limit + 1
        i = 0
        while i < min(len(typed), len(source)):
            if typed[i] != source[i]:
                break
            i += 1
        add = minimum_mewtations(typed[i:], source[i+1:], limit - 1) # Fill in these lines
        if add == 0:
            return 1
        remove = minimum_mewtations(typed[i+1:], source[i:], limit - 1)
        if remove == 0:
            return 1
        substitute = minimum_mewtations(source[i+1:], typed[i+1:], limit - 1)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    len_typed, len_source  = len(typed), len(source)
    assert len_typed <= len_source, "Words in TYPED should not more than in SOURCE."
    i = 0
    while i < len_typed:
        if typed[i] != source[i]:
            break
...
def time_per_word(words, timestamps_per_player):
    ...
    for stamps in tpp:
        times_single_player = [stamps[i] - stamps[i-1] for i in range(1, len(stamps))]
        times += [times_single_player]
    # END PROBLEM 9
